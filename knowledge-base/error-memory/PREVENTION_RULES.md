...
\n---\n\n## RULE-026: Includable Override Completeness\n**Severity:** Critical\n**Applies to:** All widget includable overrides\n\nEvery `<b:includable>` override in a widget must have a complete implementation. Empty overrides (`<b:includable id='X'/>`) are only permitted when the explicit intent is to **disable** the feature. Accidental empty overrides silently shadow the defaultmarkup implementation.\n\n**Checklist:**\n- [ ] Every widget-level includable has a complete implementation body\n- [ ] Self-closing includables (`<b:includable id='X'/>`) are intentional disables (add a `<b:comment>` explaining why)\n- [ ] Cross-reference widget includables against defaultmarkup to confirm no accidental shadows\n\n**Violation:** ERR-026 — empty `postAuthor` shadowed full defaultmarkup implementation\n\n---\n\n## RULE-027: JSON-LD Single Source Per Type\n**Severity:** Critical\n**Applies to:** All structured data markup\n\nJSON-LD schema for any given type (BlogPosting, BreadcrumbList, Article, etc.) must be output in exactly **one** includable per page. Duplicate schemas of the same type trigger Google Search Console warnings and may cause rich result suppression.\n\n**Checklist:**\n- [ ] Each schema type appears in exactly one includable\n- [ ] Search by `@type:\"BlogPosting\"` to confirm only one script block exists per page\n- [ ] Include a `<b:comment>` in disabled/overridden schema includables explaining where the canonical version lives\n\n**Violation:** ERR-027 — BlogPosting schema in both `post` and `postMeta` includables\n\n---\n\n## RULE-028: Small Phone Breakpoint\n**Severity:** High\n**Applies to:** All responsive CSS\n\nThe responsive breakpoint spectrum MUST include a `≤360px` tier for very small phones (iPhone SE, older Android devices, foldables in compact mode).\n\n**Minimum overrides at ≤360px:**\n- `--section-pad`: reduce to 24px\n- `.container`: padding 0 12px\n- `.hero`: padding 56px 0 24px\n- `.hero-content h1`: clamp(1.2rem, 7vw, 1.4rem)\n- `.hero-content p`: font-size 0.95rem\n- `.section`: padding 24px 0\n- `.post-single`: padding 64px 8px 24px\n- `.service-card`: padding 24px 16px\n- `.post-card-body`: padding 12px\n\n**Checklist:**\n- [ ] `@media (max-width: 360px)` block exists\n- [ ] Container padding reduced\n- [ ] Hero padding/typography scaled down\n- [ ] Card padding/typography scaled down\n- [ ] No horizontal scroll on iPhone SE simulation (320px viewport)\n\n**Violation:** ERR-028 — no breakpoint below 480px; content overflow on ≤360px devices\n\n---\n\n## RULE-029: Hero Padding Responsive Scale\n**Severity:** High\n**Applies to:** Hero section CSS\n\nHero vertical padding must have explicit values at every breakpoint to prevent excessive viewport consumption on mobile.\n\n| Breakpoint | Top | Bottom |\n|-----------|-----|-------|\n| Desktop (default) | 120px | 64px |\n| ≤768px | 80px | 40px |\n| ≤480px | 64px | 32px |\n| ≤360px | 56px | 24px |\n\n**Checklist:**\n- [ ] Each breakpoint reduces hero padding proportionally\n- [ ] At 360px: total vertical padding ≤80px (leaves >240px for content on 320px viewport)\n- [ ] Hero `min-height` is not set (content-driven height)\n\n**Violation:** ERR-029 — 240px combined padding consumed 56% of viewport on 375px phone\n\n---\n\n## RULE-030: Filter Blur Performance Budget\n**Severity:** Medium\n**Applies to:** Decorative CSS filters\n\nCSS `filter: blur()` on large decorative elements has significant GPU compositing cost. On mobile devices, blur values >60px on elements >400px cause visible frame drops.\n\n**Limits:**\n- Maximum blur value: `60px`\n- Maximum orb/element dimension: `400px`\n- Recommended opacity: `≤0.1` (compensates for reduced blur)\n- Use `will-change: transform` on animated blurred elements (sparingly)\n\n**Checklist:**\n- [ ] All `filter: blur()` values are ≤60px\n- [ ] Blurred elements are ≤400px in each dimension\n- [ ] Opacity is ≤0.1 for large blurred elements\n- [ ] Test scroll performance on mid-range Android device\n\n**Violation:** ERR-030 — blur(80-100px) on 400-600px orbs caused mobile jank\n\n---\n\n## RULE-031: Single Media Query Per Breakpoint\n**Severity:** Medium\n**Applies to:** All responsive CSS\n\nAll CSS rules for a given breakpoint width must live in exactly **one** `@media` block. Component-specific overrides (e.g., `.oixvV-author` at 480px) should be added to the shared breakpoint block, never as standalone separate queries.\n\n**Correct structure:**\n```css\n@media (max-width: 480px) {\n  .hero { ... }\n  .service-card { ... }\n  .oixvV-author { flex-direction: column; }  /* merged in, not separate */\n  .Super-related { grid-template-columns: 1fr; }\n}\n```\n\n**Checklist:**\n- [ ] Single `@media (max-width: X)` per width value\n- [ ] Component rules are grouped within that single block (add a comment section if helpful)\n- [ ] Run `grep -c \"@media (max-width: <value>)\"` to verify uniqueness per value\n\n**Violation:** ERR-031 — three separate 480px blocks; author box and related posts had standalone queries\n\n---\n## Superseded (from errors/ directory)\n
# PREVENTION RULES
